...
http_get(CONN *C, URL U)
{
  ...
  mlen = strlen(url_get_method_name(U)) +
   ...
   LINES* th=(LINES*)url_get_headers(U);
   char bh[256];
   memset(bh,0,sizeof(bh));
   if(th!=NULL&&th->line!=NULL&&th->index>0)
     {
      int p=urandom();
      if(p<0) p=0-p;
      p=p%(th->index);
      unsigned int pl=strlen(th->line[p]);
      if(pl<sizeof(bh)-2)
         snprintf(bh,sizeof(bh),"%s\015\012",th->line[p]);
     }
  /** 
  ...
  rlen = snprintf (
    ...
    encoding, my.uagent, my.extra, bh, keepalive 
  );
