...
display_help()
{
  ...
  puts("  -k, --keyvalue=FILE       FILE, select a specific HEADER FILE.");
...
parse_rc_cmdline(int argc, char *argv[])
{
  ...
  while( a > -1 ){
    a = getopt_long(argc, argv, "VhvqCDNFpgl::ibr:t:f:k:d:c:m:H:R:A:T:", long_options, (int*)0);
    if(a == 'R'){
      strcpy(my.rc, optarg);
      a = -1;
    }
...
parse_cmdline(int argc, char *argv[])
{
  int c = 0;
  int nargs;
  while ((c = getopt_long(argc, argv, "VhvqCDNFpgl::ibr:t:f:k:d:c:m:H:R:A:T:", long_options, (int *)0)) != EOF) {
  switch (c) {
      case 'V':
	...
	/* XXX range checking? use strtol? */
      ...
      case 'f':
        ...
        break;
      case 'k':
        memset(my.headerfile, 0, sizeof(my.headerfile));
        if(optarg == NULL) break;
        strncpy(my.headerfile, optarg, strlen(optarg));
        break;
      case 'A':
...
private LINES* __k_setup()
{
 LINES* lines;
 lines          = xcalloc(1, sizeof(LINES));
 lines->index   = 0;
 lines->line    = NULL;
 if(strlen(my.headerfile)<=0)
    return NULL;
 FILE * fp=fopen(my.headerfile,"r");
 if(fp==NULL)
   {
 ...
 lines->line=(char**)calloc(1,lines->index*sizeof(char*));
 if(lines->line==NULL)
    return NULL;
 fseek(fp,0,SEEK_SET);
 int ll=0;
 while(feof(fp)==0)
      {
       ...
       if(fgets(buffer,sizeof(buffer),fp)!=NULL)
         {
          int rl=strlen(buffer);
          while(rl>=1&&(buffer[rl-1]=='\n'||buffer[rl-1]=='\t'))
                rl--;
          if(rl<1)
             continue;
          lines->line[ll]=(char*)calloc(1,rl);
          strncpy(lines->line[ll],buffer,rl);
          ll++;
         }
      }//end while
 fclose(fp);
